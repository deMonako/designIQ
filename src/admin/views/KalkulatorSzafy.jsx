...
export default function KalkulatorSzafy({
  ...
  // Zestawienie — wszystkie materiały ze wszystkich punktów (zagregowane po nazwie)
  const allMaterials = useMemo(() => {
    const byName = {};
    Object.values(szafaPoints).forEach(pt => {
      (pt.cabinetMaterials ?? []).forEach(m => {
        if (!m.name) return;
        if (!byName[m.name]) byName[m.name] = { name: m.name, unit: m.unit, qty: 0, type: m.type };
        byName[m.name].qty = Math.round((byName[m.name].qty + m.qty) * 100) / 100;
      });
    });
              ...
              return (
                <motion.div key={tab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-2">
                  <p className="text-xs text-slate-400">
                    {isSmartHome
                      ? "Punkty z przypisanym elementem sterującym Smart Home — przypisuj materiały szafy do każdego punktu."
                      : "Punkty niesterowane (klasyczna instalacja) — przypisuj materiały szafy do każdego punktu."}
                  </p>
                  {tabRows.length === 0 ? (
                    <div className="text-center py-10 text-slate-300 text-sm border border-dashed border-slate-200 rounded-xl">
                      {isSmartHome ? "Brak punktów z elementem sterującym" : "Brak niesterowanych punktów"}
                    </div>
                  ) : (
                    <div className="border border-slate-200 rounded-xl overflow-hidden">
                      <table className="w-full text-sm">
                    ...
                    {acGroups.map((group, gi) => {
                      ...
                      const updateCircuit = (ci, patch) =>
                        setAcGroups(gs => gs.map((g, i) => i !== gi ? g : {
                          ...g, circuits: g.circuits.map((c, j) => j !== ci ? c : { ...c, ...patch })
                        }));

                      const moveGroup = (dir) => setAcGroups(gs => {
                        const arr = [...gs]; const ni = gi + dir;
                        if (ni < 0 || ni >= arr.length) return gs;
                        [arr[gi], arr[ni]] = [arr[ni], arr[gi]]; return arr;
                      });

                      const moveCircuit = (ci, dir) => setAcGroups(gs => gs.map((g, i) => {
                        if (i !== gi) return g;
                        const cs = [...g.circuits]; const ni = ci + dir;
                        if (ni < 0 || ni >= cs.length) return g;
                            ...
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="text-slate-400 font-semibold uppercase tracking-wide border-b border-slate-100">
                                  ...
                                  <th className="text-right px-3 py-1.5 w-20">Moc (W)</th>
