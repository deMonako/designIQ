    ...
    >
      ...
      {badge !== undefined && badge > 0 && (
        <span className={`text-[11px] px-1.5 py-0.5 rounded-full font-bold min-w-[20px] text-center ${
          active ? "bg-orange-500 text-white" : "bg-slate-700 text-slate-300"
        }`}>
          {badge}
        </span>
      )}
  ...
  const Sidebar = (
    <div className="flex flex-col h-full bg-slate-900 border-r border-slate-800">
      <div className="flex items-center gap-2.5 px-4 h-14 border-b border-slate-800 flex-shrink-0">
        <div className="w-7 h-7 bg-gradient-to-br from-orange-600 to-orange-500 rounded-lg flex items-center justify-center shadow-sm shadow-orange-900/50">
          <Zap className="w-3.5 h-3.5 text-white" />
        </div>
        <div>
          <div className="text-white font-bold text-sm leading-none tracking-tight">designiQ</div>
          <div className="text-slate-500 text-[10px] leading-none mt-0.5 font-medium tracking-wider uppercase">Studio</div>
        </div>
      </div>
      ...
      <div className="px-2.5 py-3 border-t border-slate-800 space-y-0.5">
        {NAV_BOTTOM.map(item => (
        ...
        <button
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-500 hover:bg-red-500/10 hover:text-red-400 transition-all border border-transparent"
        >
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col lg:ml-60 min-w-0">
        {/* Topbar */}
        <header className="h-14 bg-white border-b border-slate-200/80 flex items-center justify-between px-4 lg:px-6 flex-shrink-0 z-20">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-slate-100 text-slate-500">
              <Menu className="w-5 h-5" />
            </button>
            <div>
              <h1 className="font-bold text-slate-900 text-sm lg:text-[15px] leading-tight">
                {VIEW_TITLES[currentView] || "Panel"}
              </h1>
              <div className="hidden lg:flex items-center gap-1 text-[11px] text-slate-400 mt-0.5">
                <span>designiQ</span>
                <ChevronRight className="w-3 h-3" />
                <span className="text-slate-600">{VIEW_TITLES[currentView]}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 pl-1 pr-3 py-1.5 rounded-lg bg-slate-100">
              <div className="w-6 h-6 bg-gradient-to-br from-orange-500 to-orange-600 rounded-full flex items-center justify-center text-white text-[11px] font-bold">A</div>
              <span className="hidden sm:block text-sm font-medium text-slate-700">Adam</span>
            </div>
          </div>
        </header>
        ...
        <button
          ...
          className="fixed bottom-6 right-6 z-40 w-12 h-12 bg-gradient-to-br from-orange-600 to-orange-500 text-white rounded-full shadow-lg shadow-orange-500/30 hover:shadow-xl hover:shadow-orange-500/40 hover:scale-105 active:scale-95 transition-all flex items-center justify-center"
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
                ...
                <button onClick={() => setShowQuickAdd(false)} className="p-1 rounded-lg hover:bg-slate-100 text-slate-400">
                  <X className="w-4 h-4" />
                </button>
              </div>
